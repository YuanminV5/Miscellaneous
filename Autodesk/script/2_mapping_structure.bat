...
>nul findstr /b /e "!DirectoryID!" %resolve_conflict%  && (
  set DirectoryID=%parent_folder_name%_%DirectoryID%
  
  >nul findstr /b /e "!DirectoryID!" %resolve_conflict%  && (
	  set DirectoryID=%DirectoryID%%unique_count%
	  )
)
...
for /F "delims=" %%G in ('dir /b /a:d %folder%') do (
	...
	if !count! GTR 0 (
		...
		>nul findstr /b /e "!DirectoryID!" %resolve_conflict%  && (
			...
			set DirectoryID=%ParentDirectoryID%_!DirectoryID!
